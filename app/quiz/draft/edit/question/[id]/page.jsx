    ...
    if (latex) {
      try {
        if (quill) {
          ...
          if (range) {
            quill.insertEmbed(range.index + 1, "formula", latex);
            setLatex("");
          }
        } else if (quillAnswers) {
          const range = quillAnswers[index].getSelection(true);
          if (range) {
            quillAnswers[index].insertEmbed(range.index + 1, "formula", latex);
            setLatex("");
          }
        } else {
...
const EditQuestion = ({ params }) => {
  ...
  const getQuestion = async () => {
    setDataQuestion(await getQuestionForAdmin(params.id));
  };
  ...
  useEffect(() => {
    if (dataQuestion) {
      setPoints(dataQuestion.pointValue.toString());
      setSeconds(dataQuestion.duration.toString() / 1000);
    }
  }, [dataQuestion]);
  ...
  useEffect(() => {
    if (quill) {
      if (question) {
        ...
        setTimeout(() => {
          setSelectedFileQuestionUrl(dataQuestion?.questionImage);
        }, 0);
      }
    }
  }, [dataQuestion, quill]);
  ...
  useEffect(() => {
    if (quillAnswers) {
      dataQuestion?.options.forEach((option, index) => {
        if (quillAnswers[index]) {
          const textOption = option.textOption;

          const quillInstance = quillAnswers[index];
          quillInstance.setText("");
          quillInstance.clipboard.dangerouslyPasteHTML(
            DOMPurify.sanitize(textOption)
          );
        }
      });
    }
  }, [dataQuestion, quillAnswers]);
  ...
  const handleSubmit = async () => {
    ...
    formData.append(
      ...
      new Blob(
        [
          JSON.stringify({
            question: question,
            duration: (parseInt(seconds) * 1000).toString(),
            pointValue: parseInt(points).toString(),
    ...
    try {
      const rersponse = await editQuestion(formData, params.id);
      if (rersponse.body.message) {
        Swal.fire({
          title: "Sukses!",
          text: "Berhasil Mengubah Soal Quiz",
          icon: "success",
  ...
  return (
    <>
      <title>Mathlearn - Draft Edit Pertanyaan</title>
      {verif ? (
        <div></div>
      ) : (
        <div className={`box ${loading && "cursor-none"}`}>
          ...
          <div className="flex flex-row-reverse mt-5 mr-10 max-md:mr-0 max-md:px-5">
            <button
              onClick={() => window.history.back()}
              className="mr-5 bg-white shadow-lg rounded-xl p-4 px-8 absolute left-10 hover:bg-blue max-md:hidden text-center flex justify-center items-center"
            >
              <a>◁◁</a>
            </button>
            <div className="flex space-x-10 max-md:flex-col max-md:space-x-0 max-md:ml-0 max-md:mr-0 max-md:space-y-3 max-md:w-full">
              <button
                onClick={() => window.history.back()}
                className="md:hidden bg-grey-200 p-2 px-4 shadow-lg rounded-xl flex justify-center items-center text-center hover:bg-blue"
              >
                <a>◁◁</a>
              </button>
              <div className="relative inline-flex max-md:flex-col max-md:gap-3">
                ...
                <select
                  ...
                  className="bg-[#E1C9AD] hover:bg-orange-300 text-black font-bold py-2 px-4 rounded-2xl w-44 max-md:w-full focus:outline-none appearance-none"
                >
                  {[...Array(50).keys()].map((i) => (
            ...
            <div className="min-h-screen flex flex-col justify-center">
              <div className="relative p-4 bg-[#FF9843] shadow-lg rounded-lg">
                ...
                <div className="relative">
                  ...
                  <div
                    className={`absolute ${
                      selectedFileQuestionUrl ? "left-[49%]" : "left-[38%]"
                  ...
                  <div
                    className={`${
                      selectedFileQuestionUrl ? "flex w-[100%]" : "relative"
                    }`}
                  >
                    {selectedFileQuestionUrl && (
                      <label
