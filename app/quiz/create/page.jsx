...
import { createQuizAPI } from "../../../server/api";
...
const CreateQuiz = () => {
  ...
  const [selectedFileQuizName, setSelectedFileQuizName] = useState(null);
  const [selectedFileQuizURL, setSelectedFileQuizURL] = useState(null);
  ...
  const handleSubmit = async () => {
    ...
    formData.append("image_quiz", selectedFileQuiz);

    if (selectedFileQuestion != null) {
      formData.append("image_question", selectedFileQuestion);
    }
    ...
    // };
    // reader.readAsText(formData.get("quiz"));

    setLoading(true);

    try {
      const rersponse = await createQuizAPI(formData);
      if (rersponse.body.quizId) {
        Swal.fire({
          title: "Sukses!",
          text: "Quiz Baru Telah Terbuat",
          icon: "success",
        ...
        setTimeout(function () {
          window.location.href = `/quiz/draft/detail/${rersponse.body.quizId}`;
        }, 3000);
      }
  ...
  const handleFileChangeQuiz = async (event) => {
    const file = event.target.files[0];
    if (file) {
      const fileName = file.name;
      setSelectedFileQuizName(fileName);
      setSelectedFileQuizURL(URL.createObjectURL(file));
      setSelectedFileQuiz(file); // Simpan file yang dipilih ke state
    }
  };
  ...
  return (
    <>
      <title>Mathlearn - Membuat Quiz</title>
      ...
      {verif ? (
        ...
        <div className={`box ${loading && "cursor-none"}`}>
          ...
          <div className="border-t-8 border-orange-500 my-4"></div>
          <div>
            <div className="container mx-auto text-center">
              <div className="min-h-screen flex flex-col justify-center">
                <div className="flex max-md:mr-0 max-md:flex-col w-full">
                  <a
                    href={`/dashboard`}
                    className="bg-grey-200 p-2 shadow-lg rounded-xl text-center hover:bg-blue w-[5%]"
                  >
                    ◁◁
                  </a>
                  <div className="w-[95%]"></div>
                </div>
                <div className="flex flex-row mt-5 max-md:mr-0 max-md:flex-col w-full justify-between mb-3">
                  <input
                    type="text"
                    value={title}
                    onChange={(e) => setTitle(e.target.value)}
                    className="bg-[#E1C9AD] hover:bg-orange-300 text-black font-bold py-2 px-4 rounded-full w-[50%] max-md:w-full max-md:rounded-none focus:outline-none appearance-none placeholder:text-white"
                    placeholder="Masukan Nama Quiz"
                  ></input>
                  <div className="bg-[#E1C9AD] hover:bg-orange-300 font-bold py-2 px-4 rounded-full w-[30%] max-md:w-full max-md:rounded-none focus:outline-none appearance-none text-white flex flex-row items-center gap-5 relative">
                    <LiaImage size={30} />
                    {selectedFileQuizName && (
                      <div
                        className="absolute right-5 top-2 bottom-2 hover:text-orange-500"
                        onClick={showModal}
                      >
                        <PiMagnifyingGlassPlusLight size={30} />
                      </div>
                    )}
                    {isOpen && (
                      <Modal
                        src={selectedFileQuizURL}
                        alt="Image Quiz"
                        onClose={() => setIsOpen(false)}
                      />
                    )}
                    <label
                      htmlFor="fileInputQuiz"
                      className="flex h-full w-full cursor-pointer justify-s items-center"
                    >
                      {selectedFileQuizName ? (
                        <div className="flex flex-row">
                          <p>{selectedFileQuizName}</p>
                        </div>
                      ) : (
                        <p>Masukan Gambar Quiz [Thumbnail]</p>
                      )}
                    </label>
                    <input
                      id="fileInputQuiz"
                  ...
                  <button
                    className="bg-orange-400 hover:bg-orange-500 text-black font-bold py-2 px-4 rounded-full max-md:hidden w-[18%]"
                    onClick={handleSubmit}
                  >
                    Simpan
                  </button>
                </div>
                <div className="flex flex-row-reverse mr-10 max-md:mr-0 max-md:flex-col w-full mb-3">
                  <div className="flex flex-row mt-5 max-md:mr-0 max-md:flex-col w-full justify-between mb-3">
                    <div className="relative inline-flex w-[33%]">
                      <svg
                        className="w-2 h-2 absolute top-0 right-0 m-4 pointer-events-none color max-md:hidden"
                      ...
                      <svg
                        className="w-2 h-2 absolute top-28 right-0 m-4 pointer-events-none color md:hidden"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 412 232"
                      >
                        <path
                      ...
                      <select
                        ...
                        className="bg-[#E1C9AD] hover:bg-orange-300 text-black font-bold py-2 px-4 rounded-2xl w-full max-md:w-full max-md:rounded-none focus:outline-none appearance-none"
                      >
                ...
                <div className="relative p-4 bg-[#FF9843] shadow-lg rounded-lg">
                  ...
                  <input
                    ...
                    accept="image/*"
                    className="absolute top-5 left-5 hidden cursor-pointer max-md:hidden"
                  />
